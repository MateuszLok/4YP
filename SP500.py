...
K_star = gp.find_K_star(time_vector,new_x,sigma_f,sigma_n,length)
K_2stars = gp.find_K_2stars(new_x,sigma_f,sigma_n,length)

#Find y*
K_inv = np.linalg.inv(K)
X = np.dot(K_star,K_inv)
y_star = np.dot(X, volatility_observed)
...
new_values = np.arange(0,400,0.1)
print new_values
estimated_values_y = []
...
plt.plot(time_vector, volatility_observed, 'r')
...
plt.axis([0,len(time_vector)+100,-2.5,-0.5])
